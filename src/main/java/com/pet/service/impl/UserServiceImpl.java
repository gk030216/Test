...
public class UserServiceImpl implements UserService {
    ...
    public boolean register(RegisterRequest request) {
        // ✅ 使用Redis验证验证码（验证后自动删除）
        if (!redisUtil.verifyAndDeleteCode(request.getEmail(), request.getCode(), "register")) {
    ...
    public boolean resetPassword(ResetPasswordRequest request) {
        // ✅ 使用Redis验证验证码（验证后自动删除）
        if (!redisUtil.verifyAndDeleteCode(request.getEmail(), request.getCode(), "forget")) {
            throw new RuntimeException("验证码错误或已过期");
        }
    ...
    public boolean updateEmail(Integer userId, String currentEmail, String newEmail, String code) {
        // ✅ 使用Redis验证验证码（验证后自动删除）
        if (!redisUtil.verifyAndDeleteCode(currentEmail, code, "change_email")) {
            throw new RuntimeException("验证码错误或已过期");
        }
