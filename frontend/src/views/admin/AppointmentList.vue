<template>
  <div class="appointment-list">
    ...
    <el-table v-loading="loading" :data="appointmentList" stripe class="appointment-table">
      ...
      <el-table-column label="预约时间" width="180"><template slot-scope="scope">{{ scope.row.appointmentDate }} {{ scope.row.appointmentTime }}</template></el-table-column>
    ...
    <el-dialog title="预约详情" :visible.sync="detailVisible" width="600px" center>
      <div class="detail-content" v-if="currentAppointment">
        ...
        <div class="detail-row"><span class="label">预约时间：</span><span class="value">{{ currentAppointment.appointmentDate }} {{ currentAppointment.appointmentTime }}</span></div>
...
export default {
  ...
  methods: {
    ...
    formatDate(date) { if (!date) return ''; const d = new Date(date); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`; },
