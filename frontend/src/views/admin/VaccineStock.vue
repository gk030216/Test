<template>
  <div class="vaccine-stock">
    ...
    <el-table
      ...
      <el-table-column prop="itemName" label="疫苗名称" min-width="180"></el-table-column>
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog :title="dialogTitle" :visible.sync="showDialog" width="500px" center class="vaccine-form-dialog">
      <el-form :model="form" :rules="rules" ref="form" label-width="80px">
        <el-form-item label="疫苗名称" prop="itemName">
          <el-input v-model="form.itemName" placeholder="请输入疫苗名称"></el-input>
        </el-form-item>
        <el-form-item label="单位" prop="unit">
          <el-input v-model="form.unit" placeholder="如：支"></el-input>
        </el-form-item>
    ...
    <!-- 疫苗详情对话框 -->
    <el-dialog title="疫苗详情" :visible.sync="detailVisible" width="600px" center class="vaccine-detail-dialog">
      <div class="vaccine-detail" v-if="currentDetailItem">
        <!-- 基本信息 -->
          ...
          <el-row :gutter="16">
            <el-col :span="12">
              <div class="detail-item">
                ...
                <span class="detail-value">{{ currentDetailItem.unit || '支' }}</span>
              </div>
            </el-col>
            <el-col :span="12">
              <div class="detail-item">
    ...
    <!-- 入库对话框 -->
    <el-dialog title="入库" :visible.sync="showStockDialog" width="450px" center class="vaccine-stock-dialog">
...
<script>
import { getInventoryList, addInventory, updateInventory, addStock, deleteInventory,exportVaccineList } from '@/api/inventory';
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      this.isEdit = true;
      this.editId = row.id;
      this.form = { itemName: row.itemName, unit: row.unit || '支', stock: 0 };
      this.showDialog = true;
    },
...
/* 新增/编辑对话框样式 */
.vaccine-form-dialog ::v-deep .el-dialog {
  border-radius: 20px;
  overflow: hidden;
}

.vaccine-form-dialog ::v-deep .el-dialog__header {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  padding: 20px 24px;
  margin: 0;
}

.vaccine-form-dialog ::v-deep .el-dialog__title {
  color: white;
  font-weight: 600;
  font-size: 18px;
}

.vaccine-form-dialog ::v-deep .el-dialog__close {
  color: white;
  font-size: 20px;
}

.vaccine-form-dialog ::v-deep .el-dialog__body {
  padding: 24px;
  background: #fff;
}

/* 疫苗详情对话框样式 */
.vaccine-detail-dialog ::v-deep .el-dialog {
  border-radius: 20px;
  overflow: hidden;
}

.vaccine-detail-dialog ::v-deep .el-dialog__header {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  padding: 20px 24px;
  margin: 0;
}

.vaccine-detail-dialog ::v-deep .el-dialog__title {
  color: white;
  font-weight: 600;
  font-size: 18px;
}

.vaccine-detail-dialog ::v-deep .el-dialog__close {
  color: white;
  font-size: 20px;
}

.vaccine-detail-dialog ::v-deep .el-dialog__body {
  padding: 24px;
  background: #fff;
}
...
/* 入库对话框样式 */
.vaccine-stock-dialog ::v-deep .el-dialog {
  border-radius: 20px;
  overflow: hidden;
}

.vaccine-stock-dialog ::v-deep .el-dialog__header {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  padding: 20px 24px;
  margin: 0;
}

.vaccine-stock-dialog ::v-deep .el-dialog__title {
  color: white;
  font-weight: 600;
  font-size: 18px;
}

.vaccine-stock-dialog ::v-deep .el-dialog__close {
  color: white;
  font-size: 20px;
}

.vaccine-stock-dialog ::v-deep .el-dialog__body {
  padding: 24px;
  background: #fff;
}
