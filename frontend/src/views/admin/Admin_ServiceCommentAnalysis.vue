...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          if (res.data.commentTrend && res.data.commentTrend.length > 0) {
            this.stats.commentTrend = res.data.commentTrend.map(item => {
              let date = item.date;
    ...
    initTrendChart() {
      ...
      if (!chart) return;
      const trendData = this.stats.commentTrend || [];
      chart.setOption({
    ...
    async loadWordCloudData() {
      try {
        const res = await getServiceCommentsById(this.serviceId, { page: 1, pageSize: 200 });
...
@media (max-width: 768px) {
  .service-comment-analysis-page { padding: 12px; }
