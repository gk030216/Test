  ...
  <div class="staff-list-page">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card card-month">
        ...
        <div class="stat-info">
          ...
          <div class="stat-sub">{{ currentMonth }} 入职</div>
        </div>
      </div>
    ...
    <div class="table-card">
      ...
      >
        ...
        <el-table-column label="员工信息" min-width="210">
        ...
        <el-table-column label="入职时间" width="165" align="center">
          <template slot-scope="scope">
    ...
    >
      <div class="dialog-body">
        <div class="avatar-section">
          <div class="avatar-upload" @click="triggerUpload">
            <el-avatar :size="88" :src="avatarPreview" class="upload-avatar">
              {{ avatarPreview ? '' : (currentUser.nickname || currentUser.username || 'S').charAt(0).toUpperCase() }}
            </el-avatar>
        ...
        <el-form :model="currentUser" :rules="formRules" ref="userForm" label-width="80px" class="staff-form">
          <el-row :gutter="18">
            <el-col :span="12">
              <el-form-item label="员工账号" prop="username">
                <el-input v-model="currentUser.username" placeholder="请输入员工账号" :disabled="isEdit" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
    ...
    <el-dialog
      ...
      class="detail-dialog"
    >
      <div class="detail-body" v-if="currentDetailStaff">
        <div class="detail-avatar-area">
          <el-avatar :size="88" :src="currentDetailStaff.avatar">
            {{ currentDetailStaff.avatar ? '' : (currentDetailStaff.nickname || currentDetailStaff.username).charAt(0).toUpperCase() }}
          </el-avatar>
        </div>
        <div class="detail-grid">
          <div class="detail-item">
            <span class="d-label">员工账号</span>
            <span class="d-value">{{ currentDetailStaff.username }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">昵称</span>
            <span class="d-value">{{ currentDetailStaff.nickname || '--' }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">邮箱</span>
            <span class="d-value">{{ currentDetailStaff.email }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">手机号</span>
            <span class="d-value">{{ currentDetailStaff.phone || '--' }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">性别</span>
            <span class="d-value">{{ currentDetailStaff.gender || '保密' }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">状态</span>
            <span class="d-value">
              <span class="status-tag" :class="currentDetailStaff.status === 1 ? 'active' : 'inactive'">
                {{ currentDetailStaff.status === 1 ? '在职' : '禁用' }}
              </span>
            </span>
          </div>
          <div class="detail-item">
            <span class="d-label">入职时间</span>
            <span class="d-value">{{ formatDate(currentDetailStaff.createTime) }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">最后登录</span>
            <span class="d-value">{{ formatDate(currentDetailStaff.lastLoginTime) || '--' }}</span>
          </div>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      currentUser: {
        ...
        gender: '保密', role: 2, status: 1
      },
      stats: { total: 0, active: 0, monthNew: 0, disabled: 0 },
      formRules: {
        username: [
  ...
  methods: {
    async loadStats() {
      try {
        const res = await getUserList({ page: 1, pageSize: 9999, role: 2 });
        if (res.code === 200) {
          ...
          this.stats = {
            ...
            monthNew: list.filter(i => new Date(i.createTime) >= monthStart).length,
    ...
    async loadStaffList() {
      ...
      try {
        ...
        if (res.code === 200) {
          this.staffList = (res.data.list || []).map(item => ({ ...item, statusLoading: false }));
          this.total = res.data.total || 0;
        }
      } catch (error) {
        this.$message.error(error?.message || '加载员工列表失败');
      } finally {
        this.loading = false;
      }
    },
    handleSearch() { this.page = 1; this.loadStaffList(); },
    handleReset() {
      this.searchForm = { keyword: '', status: '' };
      this.page = 1;
      this.loadStaffList();
    },
    handlePageChange(page) { this.page = page; this.loadStaffList(); },
    handleSizeChange(size) { this.pageSize = size; this.page = 1; this.loadStaffList(); },
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          if (this.isEdit && this.currentUser.id) {
            const idx = this.staffList.findIndex(u => u.id === this.currentUser.id);
            if (idx !== -1) this.$set(this.staffList[idx], 'avatar', res.data.url);
          }
    ...
    handleAdd() {
      ...
      this.currentUser = {
        id: null, username: '', password: '', confirmPassword: '',
        email: '', phone: '', nickname: '', avatar: '', gender: '保密', role: 2, status: 1
      };
      this.avatarPreview = '';
    ...
    handleEdit(row) {
      ...
      this.currentUser = {
        ...
        gender: row.gender || '保密', role: 2, status: row.status
      };
    ...
    async batchAction(action, status = undefined) {
      ...
      try {
        await this.$confirm(`确定要${label}选中的 ${this.selectedRows.length} 个员工${suffix}`, '提示', {
          confirmButtonText: '确定', cancelButtonText: '取消',
...
.card-month .stat-icon  { background: linear-gradient(135deg, #409eff, #66b1ff); }
...
.staff-table ::v-deep .el-table__body tr.el-table__row--striped > td {
  background: #fefcf9;
}
.staff-table ::v-deep .el-table__body tr.el-table__row--striped:hover > td {
  background: #fefbf8;
}
.staff-table ::v-deep .el-checkbox__input.is-checked .el-checkbox__inner {
  background: #f59e4b;
  border-color: #f59e4b;
}
...
.staff-dialog ::v-deep .el-dialog__close { color: #fff; font-size: 22px; }
.staff-dialog ::v-deep .el-dialog__close:hover { color: rgba(255,255,255,0.7); }
.staff-dialog ::v-deep .el-dialog__body { padding: 28px; }
...
/* 表单 */
.staff-form ::v-deep .el-form-item { margin-bottom: 18px; }
.staff-form ::v-deep .el-form-item__label { font-weight: 500; color: #5c4a42; font-size: 13px; }
...
.staff-form ::v-deep .el-radio__input.is-checked .el-radio__inner {
