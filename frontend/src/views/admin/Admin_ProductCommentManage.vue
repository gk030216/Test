          ...
          <template slot-scope="scope">
            <div class="product-cell">
              <el-image :src="scope.row.productImage" class="product-thumb" fit="cover">
                <div slot="error" class="thumb-slot">
                  <i class="el-icon-picture-outline" />
                </div>
              </el-image>
              <div class="product-info">
                <span class="product-name">{{ scope.row.productName || '商品已删除' }}</span>
                <span class="product-meta">¥{{ scope.row.productPrice || '0.00' }} · ID: {{ scope.row.productId }}</span>
              </div>
            </div>
          </template>
        </el-table-column>
      ...
      <div class="detail-body" v-if="currentDetailComment">
        <div class="detail-grid">
          <div class="detail-item">
            ...
            <span class="d-value">{{ currentDetailComment.productName || '--' }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">商品价格</span>
            <span class="d-value">¥{{ currentDetailComment.productPrice || '0.00' }}</span>
          </div>
          <div class="detail-item">
            <span class="d-label">商品ID</span>
            <span class="d-value">{{ currentDetailComment.productId }}</span>
          </div>
          <div class="detail-item">
          ...
          <div class="detail-item" v-if="currentDetailComment.images">
            ...
            <span class="d-value">
              <div class="detail-images">
                <el-image
                  v-for="(img, idx) in getImageList(currentDetailComment.images)"
                  :key="idx"
                  :src="img"
                  :preview-src-list="getImageList(currentDetailComment.images)"
                  fit="cover"
                  class="detail-img"
                >
...
export default {
  name: 'Admin_ProductCommentManage',
  data() {
    return {
      loading: false,
      replyLoading: false,
    ...
    async loadStatistics() {
      try {
        ...
        if (res.code === 200) {
          ...
          const replyCount = Math.round(total * replyRate / 100);
          this.statistics = {
            total,
...
.product-name { font-weight: 500; color: #3d2e2a; }
.product-meta { font-size: 12px; color: #a08c84; }
...
.product-tag i { margin-right: 4px; }
...
@media (max-width: 768px) {
  .review-manage-page { padding: 12px; }
